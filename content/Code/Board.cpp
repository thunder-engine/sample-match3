...
class Board : public NativeBehaviour {
    A_OBJECT(Board, NativeBehaviour, Components)
...
public:
    void start() override {
        ...
        Sprite *sprite = Engine::loadResource<Sprite>("Sprites/Cell.png");
        ...
        for(uint32_t x = 0; x < width; x++) {
            for(uint32_t y = 0; y < height; y++) {
                ...
                Actor *cell = Engine::composeActor<SpriteRender>("cell", parent);
                SpriteRender *render = cell->getComponent<SpriteRender>();
                if(render) {
                ...
                Actor *actor = Engine::composeActor<Element>("gem", parent);
                Element *element = actor->getComponent<Element>();
                if(element) {
                    element->column = x;
                    element->row = y;
                    element->id = id;
               ...
               Input::touchCount() > 0) {
                ...
                if(camera) {
                    ...
                    for(uint32_t x = 0; x < width; x++) {
                        for(uint32_t y = 0; y < height; y++) {
                            if(grid[x][y] != nullptr) {
                                Transform *t = grid[x][y]->transform();
    ...
    void deleteElements(uint8_t pos, uint8_t from, uint8_t size, bool vertical) {
        score += SCORE(size);
        if(textRender) {
            textRender->setText(TString::number(score));
        }
        if(selected) {
            selected->setSelected(false);
        }
